...
meanval<-function(theta2, x1, x2, IV, v){
  
  # Convergence structure
  # The tolerance is decreasing with the level of the steps of the theta2
  ...
  # While loop, performed until we find a almsot fix point for delta. The tolerance of this 'almost' fix point will depend on the level of steps of theta2 as well as on the number of iterations already processed.
  
  i <- 0
  while ((norm>tol*10^(flag*floor(i/50)) & avgnorm>1e-3*tol*10^(flag*floor(i/50)))) {
    mval<-mvalold*s/mktsh(mvalold, expmu) # Bear in mind that here the mval and mvalold are actually exponentials, ie expmval & expmvalold. We keep this more simple notation (without exp) to not do any typos.
